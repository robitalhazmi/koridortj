...
DEFAULT_CSV_PATH = os.path.join(os.path.dirname(os.path.dirname(__file__)), "data", "transjakarta_taps.csv")
...
class TapReplayProducer:
        ...
        if self._producer is None:
            ...
            for attempt in range(1, retries + 1):
                try:
                    ...
                    logger.warning("Attempt %d/%d to connect to Kafka failed: %s", attempt, retries, exc)
                    if attempt == retries:
                        raise
                    time.sleep(2)
    ...
    def run(self) -> Dict[str, Any]:
        ...
        while True:
            ...
            pass_start_wall_dt = datetime.now(timezone.utc)
            ...
            for idx, raw_event in enumerate(events, start=1):
                ...
                if orig_tap_in and first_event_dt:
                    ...
                    sim_event_dt = datetime.fromtimestamp(sim_event_ts, tz=timezone.utc)

                    if orig_tap_out:
                        trip_duration = max(0.0, (orig_tap_out - orig_tap_in).total_seconds())
                        sim_tap_out_dt = sim_event_dt
                        sim_tap_in_dt = datetime.fromtimestamp(sim_event_ts - trip_duration, tz=timezone.utc)
                ...
                event["_streamed_at"] = datetime.now(timezone.utc).isoformat()
                ...
                if total_published % 500 == 0:
                    ...
                    logger.info("Published %d events (Throughput: %.1f events/sec)", total_published, rate)
